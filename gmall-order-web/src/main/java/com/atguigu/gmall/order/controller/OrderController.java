...
public class OrderController 
{
    ...
    public ModelAndView submitOrder(String receiveAddressId, BigDecimal totalAmount, String tradeCode, HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) 
	{
        ...
        if (success != null && success.equals("success")) //如果交易码正确
		{
            ...
            for (OmsCartItem omsCartItem : omsCartItems) 
			{
                ...
                if (b == false) //说明购物车中的价格跟 商品的价格不一样，即商品价格涨了或降了
				{
					//只要有一个价格不一样，则拒绝提交订单
    ...
    @LoginRequired(loginSuccess = true)//登录验证
    public String toTrade(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) 
	{
        ...
        List<OmsCartItem> omsCartItems = new ArrayList<>();//里面存放的是购物车数据
        ...
        /*将购物车集合转换成清单结算集合*/
        omsCartItems = cartService.cartList(memberId);//根据用户id获取购物车对象集合
		
        List<OmsOrderItem> omsOrderItems = new ArrayList<>();//里面存放的是订单详情对象
